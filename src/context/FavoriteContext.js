...
const FavoriteProvider = ({children}) => {
    ...
    const removeFavoriteCoin = async (coinId) => {
        try {
            const newList = favoriteCoinIdList.filter(favorite => favorite !== coinId);
            const jsonValue = JSON.stringify(newList);
            await AsyncStorage.setItem('@favoriteList', jsonValue);
            setFavoriteCoinIdList(newList);
            return true;
    ...
    return (
        <FavoriteContext.Provider value={{
            favoriteList: favoriteCoinIdList, getFavoriteList: getFavoriteCoinIdList, addNewFavoriteCoin, removeFavoriteCoin,
        }}>
            {children}
